...
PDF_HOST?=127.0.0.1
SITE_URL=http://${PDF_HOST}:${PORT}/clinical_genomics
PDF_DIR=_pdf
...
serve: api/swagger.json ## run a local server (You can specify PORT=, HOST=, and FLAGS= to set the port, host or to pass additional flags)
	...
	$(ACTIVATE_ENV) && \
		...
		${JEKYLL} serve --trace --strict_front_matter -d docs -P ${PORT} -H ${HOST} ${FLAGS}
.PHONY: serve
...
serve-quick: api/swagger.json ## run a local server (faster, some plugins disabled for speed)
	...
	$(ACTIVATE_ENV) && \
		mv Gemfile Gemfile.backup || true && \
		mv Gemfile.lock Gemfile.lock.backup || true && \
		${JEKYLL} serve --strict_front_matter -d docs --incremental --config _config.yml,_config-dev.yml -P ${PORT} -H ${HOST} ${FLAGS}
.PHONY: serve-quick
...
build: clean api/swagger.json ## build files but do not run a server (You can specify FLAGS= to pass additional flags to Jekyll)
	$(ACTIVATE_ENV) && \
		mv Gemfile Gemfile.backup || true && \
		mv Gemfile.lock Gemfile.lock.backup || true && \
		JEKYLL_ENV=${ENV} ${JEKYLL} build --strict_front_matter -d docs/ ${FLAGS}
.PHONY: build
...
TUTORIAL_PDFS=$(shell find docs -name 'tutorial.html' | sed 's/html$$/pdf/g')
SLIDE_PDFS=$(shell find docs -name 'slides.html' | sed 's/html$$/pdf/g')
SLIDE_PDFS+=$(shell find docs| sed 's/html$$/pdf/g')
